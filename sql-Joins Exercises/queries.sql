...
SELECT first_name, last_name, COUNT(owner_id) FROM owners LEFT JOIN vehicles ON owners.id = vehicles.owner_id GROUP BY first_name, last_name ORDER BY first_name ASC;
...
-- Count the number of cars for each owner and display the average price for each of the cars as integers. Display the owners first_name, last_name, average price 
  --and count of vehicles. The first_name should be ordered in descending order. Only display results with more than one vehicle and an average price greater 
  --than 10000. Your output should look like this:
joins_exercise=# SELECT first_name, last_name, ROUND(AVG(price)) AS avg_price, COUNT(owner_id)AS no_cars FROM owners JOIN vehicles ON owners.id = vehicles.owner_id 
GROUP BY first_name, last_name HAVING COUNT(owner_id) > 1 AND ROUND(AVG(price)) > 10000 ORDER BY first_name DESC;
 first_name | last_name | avg_price | no_cars 
------------+-----------+-----------+---------
 Shana      | Smith     |     19875 |       4
